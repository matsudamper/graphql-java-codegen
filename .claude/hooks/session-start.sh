...
python3 - <<'PYEOF'
import os, sys, hashlib, urllib.request, urllib.parse, re, subprocess, tempfile
...
# ── ダウンロード済み JDK 17 を探す ────────────────────────────────────────────
def find_jdk17_home():
    """foojay がダウンロードした JDK 17 のホームディレクトリを探す"""
    search_dirs = [
        os.path.expanduser('~/.gradle/jdks'),
        os.path.expanduser('~/.jdks/jdk-17'),
    ]
...
if jdk17_home:
    import_ca_into_jdk(jdk17_home, 'JDK 17')
    enable_basic_auth_tunneling(jdk17_home, 'JDK 17')
    write_gradle_properties(jdk_home=jdk17_home)
else:
    # JDK 17 がまだない場合：foojay Disco API から直接ダウンロードする
    import json, tarfile

    # Gradle distribution の事前ダウンロード (gradle-wrapper.properties に合わせて 8.0.2-bin)
    dist_url  = 'https://services.gradle.org/distributions/gradle-8.0.2-bin.zip'
    dist_name = 'gradle-8.0.2-bin'
    ...
    # foojay Disco API から JDK 17 を直接ダウンロード
    ...
    disco_url = (
        ...
        '&package_type=jdk&version=17&latest=overall'
    )
    print("Querying foojay Disco API for JDK 17 download URL...")
    jdk_download_url = ''
    try:
        ...
        if results:
            ...
            if pkg_id:
                ...
                print(f"Found JDK 17 package: {results[0].get('filename', 'unknown')}")
    except Exception as e:
        print(f"Disco API query failed: {e}")

    if jdk_download_url:
        jdk_tar_path = os.path.join(jdk_install_dir, 'jdk17.tar.gz')
        if not os.path.exists(jdk_tar_path):
            download(jdk_download_url, jdk_tar_path, opener)
        print(f"JDK 17 archive downloaded: {jdk_tar_path}")

        with tarfile.open(jdk_tar_path) as tf:
            tf.extractall(path=jdk_install_dir)
        import os as _os
        _os.unlink(jdk_tar_path)
        print("JDK 17 extracted")

    jdk17_home = find_jdk17_home()
    if jdk17_home:
        print(f"JDK 17 installed: {jdk17_home}")
        import_ca_into_jdk(jdk17_home, 'JDK 17')
        enable_basic_auth_tunneling(jdk17_home, 'JDK 17')
        write_gradle_properties(jdk_home=jdk17_home)
        ...
        print("Stopped old Gradle daemon")
    else:
        print("WARNING: JDK 17 could not be installed. Build may fail.")
